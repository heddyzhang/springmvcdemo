...
public class S004DownloadService {
    ...
    // 左部項目タイトル開始行
    private final int DATA_START_ROW = 0;
    // 左部項目タイトル開始列
    ...
    public void outputDownloadExcel(Workbook workbook) throws Exception {
        ...
        PoiUtil.setCellValue(PoiUtil.getCell(sheet, ymStartRow, 2), toriatsuNm);
...
//            }
    ...
    private void setMoneyData(Sheet sheet, String dataKbn, String nengetsu, int col, int kanbaiMonthFlg, String lossStartYm, Sheet styleSheet) throws Exception {
        ...
        if (isLossconData()) {
            ...
            if (("1").equals(this.dateilHeader.getAnkenEntity().getSalesClassGenka())) {
                // TODO
                setLossconInfoData(sheet, paramMap, col);
            // 進行基準の場合
            } else {
    ...
    private void setLossconInfoShinkoData(Sheet sheet, Map<String, Object> paramMap, int col) throws Exception {
        ...
        // ロスコン開始月の場合      
            ...
            lossHoseiCell.setCellFormula(PoiUtil.getCellReferenceStr(arariRuikeiRowIndex, col) + "-" + PoiUtil.getCellReferenceStr(lossHoseiHikiateNowRow.getRowNum(), col));
        ...
        // 前4半期最終月の年月セル位置(beforeQuaterLossconTotalCellIndex)を取得
        int beforeQuaterLossconTotalCellIndex = col - getQuarterMonthMinusCount(targetNengetsu);
        // ロスコン開始月もしくは(見込)四半期月もしくは売上予定月の場合に設定
        if (StringUtils.equals(targetNengetsu, lossStartYm) || isQuarter(targetNengetsu) || kanbaiMonthFlg == 1) {
            ...
            if (beforeQuaterLossconTotalCellIndex >= YM_START_COL) {
                lossconTotalCellCalc = lossconTotalCellNo + "-" + PoiUtil.getCellReferenceStr(lossHoseiHikiateTotalRow.getRowNum(), beforeQuaterLossconTotalCellIndex);
            } else {
        ...
        // ロスコンの通常月の場合
            ...
            lossHoseiHikiateNowCell.setCellFormula(null);
            // TODO
            // ロスコン引当(累計)（前4半期月の(ロスコン引当(累計額))）
            if (beforeQuaterLossconTotalCellIndex >= YM_START_COL) {
                lossHoseiHikiateTotalCell.setCellFormula(PoiUtil.getCellReferenceStr(lossHoseiHikiateTotalRow.getRowNum(), beforeQuaterLossconTotalCellIndex));
            } else {
     ...
     */
    private void setLossconInfoData(Sheet sheet, Map<String, Object> paramMap, int col) throws Exception {
        // データセット対象の年月、実績(J)/見込(M)区分、ロスコン開始年月を取得
        String targetNengetsu = (String)paramMap.get("syuekiYm");
        String dataKbn = (String)paramMap.get("dataKbn");
        String lossStartYm = (String)paramMap.get("lossStartYm");
        ...
        // TODO
        // ロスコン補正後の売上原価
        Row lossHoseiGenkaRow = sheet.getRow(lossHoseiRow.getRowNum() + 1);
        Cell lossHoseiGenkaCell = lossHoseiGenkaRow.getCell(col);
        // ロスコン粗利
        Row lossHoseiArariRow = sheet.getRow(lossHoseiGenkaRow.getRowNum() + 1);
        // ロスコンM率
        ...
        ////////////////// 以下、計算式を埋め込み(4半期月判別で計算式を埋め込む) ////////////////////////
        ///// ロスコン補正の計算式設定(完売月以外の見込月の場合に設定)
        if (kanbaiMonthFlg == 0) {
            ////// 見込月の場合のみ実施
            //// ロスコン補正 の計算式設定
            // Excelテンプレート上の通常の粗利行(粗利今回)を取得(テンプレートファイル内のセル名称を"_ARARI"としている)
            //Cell arariStartCell = PoiUtil.searchCellName(sheet, "_ARARI");　→ 2018/07/10 セル名称を付与していると、他ダウンロード機能で影響があったのでプログラムからセル行番号取得に修正
            Integer arariRowIndex = (Integer)headStartMap.get("uriGenkaKonkai") + 2;
            logger.info("arariRowIndex=" + arariRowIndex);
            Cell arariStartCell = PoiUtil.getCell(sheet, arariRowIndex, 0);

            Row arariRow;
            if (StringUtils.equals(targetNengetsu, lossStartYm)) {
                // ロスコン開始年月の場合、売上粗利累計がロスコン補正となる
                arariRow = sheet.getRow(arariStartCell.getRow().getRowNum() + 1);
            } else {
                // ロスコン開始年月以降の場合、売上粗利今回がロスコン補正となる
                arariRow = arariStartCell.getRow();
            }
            
            // ロスコン補正の計算式をセルに設定
            String refCellNo = PoiUtil.getCellReferenceStr(arariRow.getRowNum(), col);
            lossHoseiCell.setCellFormula(refCellNo);
            
            //// ロスコン補正含む売上原価の設定
            // Excelテンプレート上の売上原価(今回)の行を取得(テンプレートファイル内のセル名称を"_URI_GENKA_NOW_TOTAL"としている)
            //Cell uriageNowTotalCell = PoiUtil.searchCellName(sheet, "_URI_GENKA_NOW_TOTAL"); → 2018/07/10 セル名称を付与していると、他ダウンロード機能で影響があったのでプログラムからセル行番号取得に修正
            Integer uriageNowRowIndex = (Integer)headStartMap.get("uriGenkaKonkai");
            logger.info("uriageNowRowIndex=" + uriageNowRowIndex);
            Cell uriageNowTotalCell = PoiUtil.getCell(sheet, uriageNowRowIndex, 2);
            // ロスコン補正含む売上原価の計算式(売上原価(今回)+ロスコン補正)をセルに設定
            // 四半期粗利を計算に含めるよう変更 2018/07/05 N.Naka
            //lossHoseiGenkaCell.setCellFormula(PoiUtil.getCellReferenceStr(uriageNowTotalCell.getRow().getRowNum(), col) + "+" + PoiUtil.getCellReferenceStr(lossHoseiRow.getRowNum(), col));
            lossHoseiGenkaCell.setCellFormula(PoiUtil.getCellReferenceStr(uriageNowTotalCell.getRow().getRowNum(), col) + "+" + PoiUtil.getCellReferenceStr(lossHoseiRow.getRowNum(), col) + "-" + PoiUtil.getCellReferenceStr(lossHoseiHikiateNowRow.getRowNum(), col));
        }
 
        ///// ロスコン引当(累計),ロスコン引当(今回)の計算式設定
        if (isQuarter(targetNengetsu) || kanbaiMonthFlg == 1) {   // (見込)四半期月 or 売上予定月の場合に設定
            ////// ロスコン引当(今回)に計算式を埋め込む
            String lossconTotalCellCalc;
            String lossconTotalCellNo = PoiUtil.getCellReferenceStr(lossHoseiHikiateTotalRow.getRowNum(), col);
            // 前4半期最終月の年月セル位置(beforeQuaterLossconTotalCellIndex)を取得
            ...
            // ロスコン引当(累計),ロスコン引当(今回)の計算式をクリアする(テンプレートに予め計算式が設定されているので、それをクリアする処理となる)
            lossHoseiHikiateNowCell.setCellFormula(null);
            lossHoseiHikiateTotalCell.setCellFormula(null);
        }
    }
