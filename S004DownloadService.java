...
public class S004DownloadService {
     ...
     * 通貨種類（受注用）
     */
    private List<S004DownloadEntity> jyuchuCurrencyList;
    ...
    private void setLeftData(Sheet sheet, Sheet styleSheet) throws Exception {
        ...
        // 通貨種類(受注用)の取得        
        jyuchuCurrencyList = downloadFacade.selectJyuchuCurrencyList(paramMap);
        
        // 受注レートの出力開始行
        headStartMap.put("jyuchuSpRateStartRow", wkRow);
        //// (2018B)受注レートの通貨を埋め込んで、行を確保
        wkRow = setJyuchuCurrencyCode(sheet, wkRow, jyuchuCurrencyList, styleSheet, intCellformat, true);
        // 余分な行を削除(1行分)
        PoiUtil.delRow(sheet, wkRow, 1);
        PoiUtil.delRow(styleSheet, wkRow, 1);
        
        // 受注SPの出力開始行
        headStartMap.put("jyuchuSpKingakuStartRow", wkRow);
        //// (2018B)受注SPの通貨を埋め込んで、行を確保
        wkRow = setJyuchuCurrencyCode(sheet, wkRow, jyuchuCurrencyList, styleSheet, intCellformat, false);
        // 余分な行を削除(1行分)
        PoiUtil.delRow(sheet, wkRow, 1);
        PoiUtil.delRow(styleSheet, wkRow, 1);
...
wkRow = setCurrencyCode(sheet, wkRow, currencyList, styleSheet, null);
    ...
    */
     ...
     * @return 
     */
    private int setJyuchuCurrencyCode(Sheet sheet, int wkRow, List<S004DownloadEntity> jyuchuCurrencyList, Sheet styleSheet, DataFormat intCellformat, boolean rateFlg) {
        Cell cell;
        if (jyuchuCurrencyList == null || jyuchuCurrencyList.isEmpty()) {
            wkRow+=1;
        } else {
            // 円貨(JPY)通貨のみであるかをチェック
            boolean isEnka = isEnkaOnly(jyuchuCurrencyList);
            
            for (int idx=0; idx<jyuchuCurrencyList.size(); idx++ ) {
                String currencyCode =  jyuchuCurrencyList.get(idx).getCurrencyCode();
                ...
                // 受注レートの場合
                if (rateFlg) {
                    // 該当CELLの名称を設定（upload用）
                    PoiUtil.setCellName(sheet, "JYUCHUSP_" + currencyCode, wkRow, 3 );
                }
...
//            }
    ...
    private void setMoneyData(Sheet sheet, String dataKbn, String nengetsu, int col, int kanbaiMonthFlg, String lossStartYm, Sheet styleSheet) throws Exception {
        ...
        // 年月に関連する受注情報を設定
        setJyuchuDataByYm(sheet, paramMap, col);
     ...
     * @param colIdx 
     */
    public void setJyuchuDataByYm(Sheet sheet, Map<String, Object> paramMap, int colIdx) throws Exception {

        int rowSpRateIdx = 0;
        int rowSpKingakuIdx = 0;
        Cell spTotalCell;
        String totalSpFormula = "";
        String rateCellRef;
        Cell cell;
        
        // 受注SP情報リストを取得
        List<Map<String, Object>> jyuchuSpMapList = downloadFacade.selectJyuchuSpMapList(paramMap);
        
        for (int idx = 0; idx < jyuchuSpMapList.size(); idx++) {
            
            // 受注SP情報
            Map<String, Object> jyuchuSpData = jyuchuSpMapList.get(idx);
            // 受注レート行番号を取得
            rowSpRateIdx = (int) headStartMap.get("jyuchuSpRateStartRow") + idx;
            // 受注レート行の内容を設定
            PoiUtil.setCellValue(sheet, rowSpRateIdx, colIdx, jyuchuSpData.get("JYUCHU_RATE"));
            cell = PoiUtil.getCell(sheet, rowSpRateIdx, colIdx);
            cell.setCellFormula(null);
            //PoiUtil.setCellValue(sheet, rowSpRateIdx, colIdx, Utils.changeBigDecimal(Utils.getObjToStrValue(jyuchuSpData.get("JYUCHU_RATE"))));

            // 受注SP行番号を取得
            rowSpKingakuIdx = (int) headStartMap.get("jyuchuSpKingakuStartRow") + idx;
            // 受注SP行の内容を設定
            PoiUtil.setCellValue(sheet, rowSpKingakuIdx, colIdx, jyuchuSpData.get("JYUCHU_SP"));
            //PoiUtil.setCellValue(sheet, rowSpRateIdx, colIdx, Utils.changeBigDecimal(Utils.getObjToStrValue(jyuchuSpData.get("JYUCHU_SP"))));
            cell = PoiUtil.getCell(sheet, rowSpKingakuIdx, colIdx);
            cell.setCellFormula(null);
            
            // 日本円の場合
            if (ConstantString.currencyCodeEn.equals(jyuchuSpData.get("CURRENCY_CODE"))){
                // 受注レートが1
                rateCellRef = "1";
            // 外貨の場合
            } else {
                // 受注レートのセール引用
                rateCellRef = PoiUtil.getCellReferenceStr(rowSpRateIdx, colIdx);
            }
            
            // 最初の何番目の場合
            if (idx == 0) {
                // 受注SP * 受注レート
                totalSpFormula = PoiUtil.getCellReferenceStr(rowSpKingakuIdx, colIdx) + "*" + rateCellRef;
            // その以降の場合
            } else {
                // 受注SP1 * 受注レート1, 受注SP2 * 受注レート2,...
                totalSpFormula = totalSpFormula + "," + PoiUtil.getCellReferenceStr(rowSpKingakuIdx, colIdx) + "*" + rateCellRef;
            }
        }
    	 
        // 受注SP合計のセールを取得
        spTotalCell = PoiUtil.getCell(sheet, (int) headStartMap.get("jyuchuSpKingakuStartRow") + jyuchuSpMapList.size(), colIdx);
        // 計算公式を設定
        spTotalCell.setCellFormula("ROUND(SUM(" + totalSpFormula + "), 2)");
